...
final class AppState: ObservableObject {
    ...
    private init() {
        // Must run before any store computes its folder or reads UserDefaults,
        // so notes and settings from the old "Earshot" identity are already in
        // place under "Oats" by the time the stores initialize.
        DataMigration.run()
    ...
    func bootstrap() {
        ...
        // QA hook: OATS_QA_ONBOARD=1 forces the first-run onboarding for a
        // screenshot WITHOUT clearing the real didOnboard flag.
        if ProcessInfo.processInfo.environment["OATS_QA_ONBOARD"] == "1" {
            showOnboarding = true
        }
        // QA hook: OATS_QA_COMPANION=1 runs the exact HUD start path (real
        ...
        if let qa = ProcessInfo.processInfo.environment["OATS_QA_COMPANION"], qa == "1" || qa == "2" {
            Task { @MainActor in
                func frames() -> String {
                    NSApp.windows
        ...
        if ProcessInfo.processInfo.environment["OATS_QA_SCROLL"] == "1" {
            ...
            _ = NSEvent.addLocalMonitorForEvents(matching: .scrollWheel) { event in
                ...
                if let data = line.data(using: .utf8) {
                    if let handle = FileHandle(forWritingAtPath: "/tmp/oats-scrollevents.txt") {
                        handle.seekToEndOfFile()
                        handle.write(data)
                        try? handle.close()
                    } else {
                        try? data.write(to: URL(fileURLWithPath: "/tmp/oats-scrollevents.txt"))
                    }
                }
                return event
            }
            Task { @MainActor in
                ...
                try? log.write(toFile: "/tmp/oats-scroll.txt", atomically: true, encoding: .utf8)
            }
        }
        // QA hook: OATS_QA_GRAPH=1 opens the knowledge graph on launch so
        // the canvas layout can be screenshotted.
        if ProcessInfo.processInfo.environment["OATS_QA_GRAPH"] == "1" {
            Task { @MainActor in
                try? await Task.sleep(for: .seconds(1))
                self.sidebar = .graph
            }
