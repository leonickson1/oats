...
// The brain. Oats has no API keys and no cloud account: it pipes transcripts
...
enum AgentPrompts {
    ...
    static func ask(question: String, history: [ChatMessage], notes: [(meta: NoteMeta, summary: String)]) -> String {
        ...
        return """
        You are Oats's assistant, running locally on this Mac. Answer the user's question directly and helpfully. If the question is about their meetings, use the meeting summaries below; otherwise just answer normally. Format the answer in clean markdown (headings, bullets, bold where it helps).
